...
def prepareData(scaling='none', removed_features=[]):
    ...
    # Encode labels
    le = LabelEncoder()
    y_train = train_labels.apply(le.fit_transform)
    y_test = test_labels.apply(le.transform)
    
    # Export scaler and encoder and return transformed datasets
    joblib.dump(ohe, '../preprocessing/onehotencoder.gz')
    joblib.dump(le, '../preprocessing/labelencoder.gz')
    if scaling == True:
        joblib.dump(scaler, '../preprocessing/scaler.gz')

    return x_train, y_train['label'], x_test, y_test['label']
